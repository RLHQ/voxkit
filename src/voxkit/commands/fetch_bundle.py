"""voxkit fetch-bundle — 从 GitHub Release 拉模型 bundle 解到本机 HF cache。

数据源优先级：
  1. ``--bundle PATH`` —— 本地已有 tar.gz（开发或离线分发）
  2. ``--from URL`` —— 任意 HTTPS URL
  3. ``--release TAG`` 默认走 GitHub Release（``3Craft/voxkit``）
...
def _extract_tarball(bundle: Path, hub: Path, *, force: bool, manifest: BundleManifest) -> None:
    ...
    bundle 内顶层是 ``ATTRIBUTION.md`` + 若干 ``models--xxx--yyy/`` + 可选 ``aux/``，
    其中前两类正好对应 hub 下的格式。``aux/`` 会暂时落在 ``hub/aux/``，由
    ``_install_aux_files`` 搬到 manifest 指定的 target_path 后删除。
...
# ── aux 文件安装 ─────────────────────────────────────────────────────────
def _install_aux_files(hub: Path, manifest: BundleManifest, *, force: bool) -> int:
    """把已解压到 ``hub/aux/`` 的辅助文件搬到 manifest 中指定的 target_path。

    流程：
      1. 校验 ``hub/aux/<filename>`` 存在 + sha256
      2. target_path 已存在且 ``--force`` 未设：跳过（打印 skipped）
      3. 否则 mkdir 父目录，shutil.copy2 覆盖，校验落地后 sha256
      4. 清理 ``hub/aux/``（即便部分跳过也清理，因为源文件已不需要了）

    返回非零 = 校验失败计数。
    """
    if not manifest.aux_files:
        # 即便没有 aux_files，hub/aux/ 不应当出现；防御性清理
        stale = hub / "aux"
        if stale.is_dir():
    ...
    aux_staging = hub / "aux"
    failed = 0
    for entry in manifest.aux_files:
        src = aux_staging / entry.filename
        target = Path(entry.target_path).expanduser()

        if not src.is_file():
            print(f"❌ aux 缺失: {entry.filename}（bundle 中应有 aux/{entry.filename}）",
                  file=sys.stderr)
            failed += 1
            continue
        ...
        if actual != entry.sha256:
            print(f"❌ aux sha256 不匹配: {entry.filename}", file=sys.stderr)
            failed += 1
            continue

        if target.exists() and not force:
            print(f"[fetch] aux {entry.name}: skipped (已存在 {target}; --force 可覆盖)")
            continue

        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(src, target)
        print(f"[fetch] ✅ aux {entry.name} → {target}")

    # 清理 staging
    if aux_staging.is_dir():
        shutil.rmtree(aux_staging, ignore_errors=True)
    return failed
...
def run(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="voxkit-fetch-") as tmpd:
        tmp = Path(tmpd)
        bundle_path: Path
        manifest_path: Path
        ...
        try:
            ...
            return int(ExitCode.GENERIC_FAIL)
        print(f"[fetch] manifest: voxkit={manifest.voxkit_version} "
        ...
        # ── aux 文件安装（搬到 target_path）─────────────────────────
        aux_failed = _install_aux_files(hub, manifest, force=args.force)
        if aux_failed:
            print(f"❌ {aux_failed} 个 aux 文件安装失败", file=sys.stderr)
            return int(ExitCode.GENERIC_FAIL)
    ...
    print(f"\n✅ 模型已就绪。运行 'voxkit doctor' 验证；可直接 'voxkit diarize ...'")
