...
为什么是独立 worker？
- voxkit CLI 主进程零 pyannote 依赖（pipx 装即可秒启）
- pyannote / torch / torchaudio 走独立 venv (~/.local/share/voxkit/venv/)
- spawn `<venv>/bin/python -m voxkit.core.pipeline ...` 跑这个文件

输入：CLI args（音频路径 / model / device / num_speakers ...）
输出：把 DiarizationOutput JSON 写到 stdout（最后一行）；进度走 stderr NDJSON

注意：本模块在主进程 import 不会 fail（只 import stdlib + voxkit.io），
...
from voxkit import SCHEMA_VERSION
...
def run_diarize(
    *,
    ...
    from voxkit.core.device import select_device
...
def _build_worker_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="voxkit-pipeline-worker")
