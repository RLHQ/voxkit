...
8. **保留切坏的尾部**：源字幕可能在介词/连词/助动词后被切（例如末尾停在 "got some" / "is it" / "I'll"）。翻译时**保留**这种不完整尾部，**不要**补造完整句子让目标段落看起来语法独立——下一条字幕会自然衔接。最多用逗号收尾
9. **跨 cue 连读自然**：如果两条相邻字幕在源语言里是连续语义、被切成两段，目标语言两段也要可以连读——不要在第一段强行画句号让两段读起来割裂。每段单独读应该可懂，连起来读应该流畅
