"""voxsplit diarize — 主命令。

流程：
  1. 验证输入文件 → 抽音频（视频）/ 直接用（音频）
  2. ensure_venv 触发 lazy install（首次自动）
  3. spawn `<venv>/bin/python -m voxsplit.core.pipeline ...`，把 worker 的 stdout
...
from voxsplit.commands import align as align_cmd
from voxsplit.core import audio as core_audio
from voxsplit.core import env as core_env
from voxsplit.core import lazy_install
from voxsplit.core.constants import ExitCode, WORKER_JSON_SENTINEL
from voxsplit.io.progress import ProgressEmitter
from voxsplit.io.schema import DiarizationOutput
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        progress.error("LAZY_INSTALL_FAILED", str(e),
                       fix="手动跑 voxsplit setup 排查；或 rm -rf ~/.local/share/voxsplit 重来")
        return int(ExitCode.GENERIC_FAIL)
