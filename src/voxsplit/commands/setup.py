"""voxsplit setup — 显式触发 lazy install 等同流程。

逻辑等价于第一次跑 diarize 时的自动安装：
  1. 创建 ~/.local/share/voxsplit/venv（uv，Python 3.12）
  2. 安装 pyannote.audio 4.x + 当前 voxsplit 包（带 worker extra）
  3. 写标记文件 ~/.cache/voxsplit/.installed
...
def run() -> int:
    ...
    print("[setup] 检查并准备 voxsplit venv...")
    try:
