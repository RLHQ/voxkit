"""voxsplit build-bundle — 把本机 HF cache 中 voxsplit 用到的 4 个模型打包。

输出：
- ``voxsplit-models.tar.gz`` —— 主 bundle，含 4 个 ``models--pyannote--*`` 目录
  + ``ATTRIBUTION.md``（CC-BY-4.0 强制要求）
- ``voxsplit-models.manifest.json`` —— 独立 release asset，含每文件 sha256 +
  bundle 整体 sha256，供 fetch-bundle 校验

Bundle 内部结构（保留 HF cache 标准）：
    voxsplit-models.tar.gz
    ├── ATTRIBUTION.md
    └── hub/
...
（whisper / transformers / datasets ...）—— 必须**严格只打包 voxsplit 必需的
...
def _tar_create(
    ...
    with tempfile.TemporaryDirectory(prefix="voxsplit-bundle-") as tmpd:
        staging = Path(tmpd)
        (staging / "ATTRIBUTION.md").write_text(attribution_text)

        cmd = [
            "tar", "-c", "-z", "-f", str(bundle_path),
            "-C", str(staging), "ATTRIBUTION.md",
...
def run(args: argparse.Namespace) -> int:
    ...
    for spec in BUNDLE_MODELS:
        ...
        if not model_dir.is_dir():
            ...
            print(f"   提示：先跑一次 voxsplit diarize 让模型下载到 HF cache", file=sys.stderr)
            return int(ExitCode.GENERIC_FAIL)
        try:
    ...
    manifest = BundleManifest(
        schema_version="1",
        voxsplit_version=voxsplit.__version__,
    ...
    print(f"     --repo 3Craft/voxsplit --notes 'voxsplit models bundle {bundle_version}'")
...
def add_subparser(subparsers) -> None:
    p = subparsers.add_parser(
        "build-bundle",
        help="把本机 HF cache 中的 voxsplit 必需模型打包成可分发 tar.gz",
    )
