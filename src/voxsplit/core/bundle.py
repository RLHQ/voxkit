...
class BundleManifest(BaseModel):
    """voxsplit 模型 bundle 顶层 manifest，作为单独 release asset 与 .tar.gz 一起发布。"""

    model_config = ConfigDict(populate_by_name=True)

    schema_version: str = Field("1", alias="schemaVersion")
    voxsplit_version: str = Field(..., alias="voxsplitVersion")
...
ATTRIBUTION_TEMPLATE = """# voxsplit Models Bundle — Attribution
...
- This bundle was produced by `voxsplit build-bundle` (voxsplit
  {voxsplit_version}, bundle {bundle_version}).
"""
...
def render_attribution(manifest: BundleManifest) -> str:
    ...
    return ATTRIBUTION_TEMPLATE.format(
        model_blocks="\n".join(blocks).rstrip(),
        voxsplit_version=manifest.voxsplit_version,
