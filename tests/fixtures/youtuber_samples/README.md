...
|---|---|---|---|---|---|
| `xnzxnz_first_look_10min/` | 小宁子 XNZ | 10:06 | 按需 `fetch.sh` | zh-Hans, zh-Hant, en-US（**zh↔en 同帧对齐**） | 中文 reseg + zh↔en translate 金标 |
| `kurzgesagt_germany_14min/` | Kurzgesagt | 14:05 | 按需 `fetch.sh` | en（**作者人工**） | 英文短篇科普 proofread / reseg |
...
字幕全部入仓；所有音频按需 `./fetch.sh` 拉取（不入仓，避免第三方音频版权问题）。
